...
65. **修復 LLM 括號字元替換導致 placeholder 洩漏**（v0.93 修正）：LLM 有時會把 `⟦⟧`（U+27E6/U+27E7）替換成外觀相似但 Unicode 不同的 `❰❱`（U+2770/U+2771 Heavy Angle Bracket Ornament），導致 `parseSegment` 的 regex 完全認不出標記。修正：`normalizeLlmPlaceholders` 新增「第零步」——把已知的 LLM 替代括號還原成標準 `⟦⟧`，透過 `BRACKET_ALIASES_OPEN/CLOSE` 陣列管理，未來觀察到新的替代字元只需加一行。`stripStrayPlaceholderMarkers` 的第三步也同步擴充，清除替代字元的孤立殘留。實測 Edo Wikipedia 頁面：修正前 12 個 text node 含 `❰❱` 標記（集中在參考文獻區的 `<cite>` 巢狀結構）；修正後 0 洩漏。涉及檔案：`content.js`。

66. **Temperature 預設值降低 + Mismatch 黃色閃爍進度條 + 設定頁儲存狀態提示**（v0.94 更新）：三項 UI / 設定改善。（a）**Temperature 預設值從 1.0 降為 0.5**——使用者觀察到低效能模型在 Temperature > 0.5 時錯誤率顯著上升，降低預設值提升翻譯穩定性。涉及 `lib/storage.js` DEFAULTS、`options/options.js` DEFAULTS、`options/options.html` 說明文字。（b）**Mismatch 黃色閃爍進度條**——當翻譯批次發生 segment mismatch（LLM 回傳段數不符，觸發逐段 fallback）時，Toast 進度條從藍色改為黃色閃爍（`animation: blink-yellow .6s`），讓使用者知道翻譯正在走 fallback 路徑。`hadMismatch` 旗標從 `lib/gemini.js` `translateChunk` → `translateBatch` → `background.js` → `content.js` `onProgress` → `showToast` 全鏈路傳遞。涉及 `lib/gemini.js`、`background.js`、`content.js`。（c）**設定頁儲存狀態提示條**——頁面上方新增狀態條：任何設定欄位有未儲存變更時顯示紅色「有未儲存的變更」；按下儲存後改為綠色「設定已儲存」（3 秒後自動隱藏）。使用 event delegation 監聽 `#tab-settings` 的 `input`/`change` 事件偵測變更。涉及 `options/options.html`、`options/options.css`、`options/options.js`。純 UI / 設定層改動，不影響段落偵測 / 注入邏輯，依 CLAUDE.md 硬規則 9 不需 regression spec。

72. **全面移除 Thinking 功能**（v1.00 更新）：Gemini REST API 的非串流 `generateContent` 端點實測無法回傳 `thoughtsTokenCount`（即使完全不設定 `thinkingConfig`、模型預設 high 等級動態思考，API 回應的 `usageMetadata` 始終不含此欄位），追蹤程式碼無法驗證，投資報酬率過低，決定全面拔除。移除範圍：（a）`lib/gemini.js`：移除 `useThinking` 解構、`thoughtsTokens` 欄位（translateBatch/translateChunk/fallback aggUsage）、thought part 過濾邏輯、debug log（raw response structure / thinkingConfig 相關）。翻譯主路徑（`translateChunk`）與術語表（`extractGlossary`）均保留 `thinkingConfig: { thinkingBudget: 0 }`（明確關閉思考，避免思考 token 吃掉 maxOutputTokens 額度導致 MAX_TOKENS 截斷）。（b）`background.js`：`computeCostUSD` 與 `computeBilledCostUSD` 移除 `thoughtsTokens` 參數，handleTranslate/handleExtractGlossary 對應呼叫簡化。usage 回傳物件移除 `thoughtsTokens`。（c）`content.js`：`pageUsage` 移除 `thoughtsTokens`，toast 與 LOG_USAGE 的 billedTotalTokens 公式回歸 `billedInputTokens + outputTokens`。（d）`lib/usage-db.js`：移除 `totalThoughtsTokens` 彙總、`getAggregated` totalTokens 公式簡化、CSV 移除「思考 tokens」欄位。（e）`options/options.js`：移除 `isThinkingSelected()`、`__thinking` 後綴處理邏輯、`useThinking` 儲存與驗證、用量顯示的 thoughtsTokens。（f）`options/options.html`：移除 Thinking 模型變體選項。（g）`lib/storage.js`：移除 `useThinking` 預設欄位。純功能移除，不影響段落偵測 / 注入邏輯，不需 regression spec。

70. **Thinking 改用 thinkingLevel + 移除 Gemini 2.5 Flash**（v0.98 更新，v1.00 已全面移除 Thinking）：歷史紀錄保留。

69. **Thinking 整合進模型選單 + 移除獨立 checkbox + 移除 2.5 Flash Lite 與 2.5 Pro**（v0.97 更新，v1.00 已全面移除 Thinking）：歷史紀錄保留。

68. **預設值調整 + Rate limit 對照表全面更新**（v0.96 更新）：（a）`lib/storage.js` DEFAULT_SETTINGS 的 `autoTranslate` 從 `true` 改為 `false`、`glossary.enabled` 從 `true` 改為 `false`，僅影響全新安裝或清除設定後的預設狀態。（b）`lib/tier-limits.js` 與 `options/options.js` 的 TIER_LIMITS 對照表依 2026-04 AI Studio 實際數值全面更新。主要差異：Tier 1 flash-lite RPM 300→4000、TPM 2M→4M、RPD 無限制；flash RPM 300→1000、TPM 2M→1M、RPD 1500→10000；Pro TPM 1M→2M；3.1-pro RPM 150→225、RPD 1000→250。Tier 2 各模型亦大幅調升。Unlimited RPD 以 `Infinity` 表示，rate limiter 邏輯可正確處理（`count > Infinity` 永遠 `false`）。設定頁 RPD 欄位遇到 Infinity 時顯示「無限制」。純設定層改動，不需 regression spec。

67. **設定頁儲存提示條改為 fixed 定位**（v0.95 修正）：v0.94 的儲存狀態提示條放在 `.container` 內部，捲動後會被捲出視野。改為 `position: fixed; top: 0; left: 0; right: 0; z-index: 9999`，固定在頁面最頂端，確保任何捲動位置都能看到紅色「有未儲存的變更」/ 綠色「設定已儲存」提示。HTML 結構從 `.container` 內移到 `<body>` 直接子元素。純 UI 改動，不需 regression spec。

64. **修復 placeholder 標記洩漏至可見 DOM**（v0.92 修正）：LLM 有時會丟掉佔位符的開頭 `⟦`（U+27E6）但保留結尾 `⟧`（U+27E7），導致 `⟧`、`/N⟧`、`*N⟧` 等半截標記殘留在翻譯後的可見文字中。根因：`stripStrayPlaceholderMarkers()` 的 regex 只匹配完整 `⟦...⟧` 模式，orphan `⟧` 不會被清除。修正：三步驟剝除——(1) 完整標記 `⟦0⟧`/`⟦/0⟧`/`⟦*0⟧`；(2) 半截標記 `/N⟧`、`*N⟧`；(3) 殘留的孤立 `⟦` 或 `⟧`。同時將 `injectTranslation` fallback 路徑的 inline regex 統一改用 `stripStrayPlaceholderMarkers()`，確保所有注入路徑共用同一套清理邏輯。`⟦`/`⟧` 是數學方括號，自然語言文本幾乎不使用，清除安全。實測 Edo Wikipedia 頁面：修正前 22 個 text node 含 32 個 orphan `⟧`；修正後 0 洩漏。涉及檔案：`content.js`。

63. **降低 batch 段數上限，減少 segment mismatch**（v0.91 調整）：`content.js` 的 `MAX_UNITS_PER_BATCH` 與 `lib/gemini.js` 的 `MAX_UNITS_PER_CHUNK` 從 20 降為 12。字元預算 `MAX_CHARS_PER_BATCH = 3500` 不變。原因：v0.90 的 11 輪自動測試顯示 82% 的輪次有至少 1 個 batch mismatch，其中 `expected=20` 的大 batch 佔絕大多數。段數越多，模型越難追蹤分隔符邊界。降低段數上限後 batch 數會增加，但有 `maxConcurrent=10` 並行消化，預期總時間不增反降（因為省掉大量 fallback 重送）。涉及檔案：`content.js`、`lib/gemini.js`。

62. **RPD 改為軟性預算警告 + rate limiter 重構**（v0.90 修正）：(1) **rate-limiter.js 完整重構**——移除 priority queue + `dispatchLoop` 架構（`Promise.resolve().then()` 和 `setTimeout(0)` 排程的 microtask/macrotask 在 Chrome Service Worker 環境中會被吞掉不執行，導致所有翻譯請求卡在 `acquire()` 永遠不 resolve）。新架構：每個 `acquire()` 自己 inline while loop 檢查 `computeWaitMs()`，不依賴外部 dispatcher。代價：失去 p0/p1 priority 排序，但實際影響極小。(2) **RPD 改為軟性預算警告**——`acquire()` 不再因 RPD 超限而阻擋翻譯。RPM/TPM 仍為硬限制（避免短時間大量請求觸發 429），RPD 改為只追蹤計數並回傳 `rpdExceeded` 旗標。真正的每日限額由 Gemini API 自己回傳錯誤。`computeWaitMs()` 移除 RPD 檢查。(3) **content.js 軟性 RPD 警告 Toast**——翻譯完成後若偵測到 `rpdExceeded`，延遲 1.5 秒顯示「提醒：今日 API 請求次數已超過預算上限」，6 秒自動消失，不阻擋任何操作。(4) **Debug Bridge 新增 `CLEAR_RPD` action**——測試用，可遠端清除 RPD 計數。涉及檔案：`lib/rate-limiter.js`（重構）、`background.js`（新增 CLEAR_RPD handler + rpdExceeded 回傳）、`content.js`（RPD 警告 Toast + CLEAR_RPD bridge）。

61. **多段翻譯序號標記降低 segment mismatch**（v0.89 新增）：`lib/gemini.js` 的 `translateChunk()` 在多段批次翻譯時，為每段輸入文字加上 `«N» ` 序號前綴（N = 1 到段數），並在 system prompt 追加明確指令要求模型在輸出時保留對應序號。解析端以 `SEQ_MARKER_RE = /^«\d+»\s*/` 移除序號標記。目的：透過序號讓 LLM 更容易追蹤段落邊界，減少分隔符遺漏或合併段落的機率，降低 per-segment fallback 的觸發頻率。單段翻譯不加序號（避免無謂 token 開銷）。涉及檔案：`lib/gemini.js`。

60. **內建 Log 檢視頁**（v0.88 新增）：設定頁新增第三個分頁「Log」，取代原本需要開 Chrome DevTools 查看 extension log 的工作流程。架構：(1) **記憶體 buffer**——`lib/logger.js` 全面重寫，從 `chrome.storage.local` 持久化改為純記憶體環形 buffer（上限 1000 筆），每筆 log 帶 `seq`（遞增序號）、`t`（ISO 時間戳）、`level`（info/warn/error）、`category`（translate/api/cache/rate-limit/glossary/spa/system 七類）、`message`、`data`（結構化附加資料）。Extension reload 或 Service Worker 回收時 buffer 自動清空（設計上 log 只用於當次除錯，不需持久化）。(2) **統一 log 收攏**——`background.js`、`lib/gemini.js`、`lib/cache.js`、`lib/rate-limiter.js` 的所有 `console.log/warn/error` 全部替換為 `debugLog(level, category, message, data)` 四參數呼叫。`content.js`（不能 import ES module）新增 `sendLog()` wrapper，透過 `chrome.runtime.sendMessage({ type: 'LOG' })` 把 log 轉發到 background 寫入 buffer。(3) **差異輪詢**——設定頁 Log 分頁啟用時每 2 秒向 background 送 `GET_LOGS` 訊息（帶 `afterSeq` 參數），只拉回上次之後的新 log，前端累積到 2000 筆上限。切離 Log 分頁時停止 polling。(4) **UI 功能**——分類篩選（7 類 + 全部）、等級篩選（info/warn/error + 全部）、關鍵字搜尋（同時搜尋 message、category、data JSON）、清除（同步清除前後端 buffer）、匯出 JSON（下載目前篩選結果或全部 log）、自動捲動開關、data 展開/收合（點擊 `{…}` 按鈕展開結構化資料的 JSON 格式化檢視）。(5) **DevTools Console 輸出可選**——設定頁「除錯」區塊的勾選改為「同時將 Log 輸出到 DevTools Console（進階開發者用）」，預設關閉。開啟時 `debugLog()` 會在寫入 buffer 之外同步 `console.log/warn/error` 到 DevTools。涉及檔案：`lib/logger.js`（重寫）、`background.js`（新增 LOG/GET_LOGS/CLEAR_LOGS handler + 全面 debugLog 收攏）、`lib/gemini.js`（debugLog 四參數）、`lib/cache.js`（新增 debugLog import + 收攏）、`lib/rate-limiter.js`（debugLog 四參數）、`content.js`（新增 sendLog wrapper + 收攏）、`options/options.html`（Log 分頁 HTML）、`options/options.css`（Log 分頁樣式）、`options/options.js`（Log 分頁邏輯）。此功能為 UI + logging 改動，不影響翻譯核心行為，不需 regression spec。
...
- ~~**內建 Log 檢視頁**：已於 v0.88 實作完成（見 §2.1 #60）~~
- **SPA 支援調優與 bug 修復**：v0.82 的 SPA 基礎架構（pushState/replaceState 偵測 + MutationObserver 追加翻譯）已上線，但實際使用中已觀察到多處問題，需要系統性排查與修正
...
Log 輸出到：
1. 設定頁「Log」分頁（v0.88 實作）：記憶體 buffer 最近 1000 筆，支援分類/等級篩選、關鍵字搜尋、匯出 JSON、清除、自動捲動。差異輪詢（每 2 秒，帶 `afterSeq`），只在 Log 分頁啟用時 polling
2. 瀏覽器 DevTools Console（可選）：設定頁「除錯」勾選「同時將 Log 輸出到 DevTools Console」後啟用，預設關閉
